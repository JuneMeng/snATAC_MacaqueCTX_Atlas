...
disk_cache = Cache(directory="/cluster/share/atac_group/mafas5/chen_ws/SST/celloracle/tmpdir")
...
work_dir = '/cluster/share/atac_group/mafas5/chen_ws/SST/celloracle'

parser = argparse.ArgumentParser(description = "run GRN")
parser.add_argument("--tfdir", type = str, default = work_dir)
parser.add_argument("--scRNAdir", type = str, default =  "/cluster/share/atac_group/mafas5/chen_ws/SST/celloracle/RNA/layer")
parser.add_argument("--outdir", type = str, default = os.path.join(work_dir, "GRN"))
parser.add_argument("--group", type = str)
parser.add_argument("--groupcol", type = str, default = "layer")
...
adata = sc.read_h5ad(f"{args.scRNAdir}/SST.layer.{group}.ann.hdf5")
if baseGRN_all > 0:
    print("Use global baseGRN from SST-layer.all.baseGRN.df.parquet.")
    tfi_all = mycelloracle.load_tfidf(
        f"{args.tfdir}/SST-layer.all.baseGRN.df.parquet")
else:
    print(f"Use {group} baseGRN from sa2subclass.all.baseGRN.df.parquet.")
    tfi_all = mycelloracle.load_tfidf(
        f"{args.tfdir}/{group}.pdc.baseGRN.df.parquet"
    )

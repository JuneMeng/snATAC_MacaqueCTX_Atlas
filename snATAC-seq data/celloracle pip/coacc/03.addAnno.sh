...
fitConn_file="${input_dir}/${group}-fitConns.res.txt"
if [ ! -f "${fitConn_file}" ]; then
    echo -e "${fitConn_file} does not exist."
    exit 1
fi

fitConn_alignv1_file="${cicero_dir}/${group}.fitConns.res.alignv1.txt"
cut -f 1,2,3,5,6 ${fitConn_file} > ${fitConn_alignv1_file}

out_anno="${cicero_dir}/${group}.fitConns.res.anno"
out_sel="${cicero_dir}/${group}.fitConns.res.sel"
out_sta="${cicero_dir}/${group}.conns.sta"
out_pdc="${cicero_dir}/${group}.fitConns.res.pdc"
out_alignv1_pdc="${cicero_dir}/${group}.fitConns.res.alignv1.pdc"
out_bedpe="${cicero_dir}/${group}.pdc.bedpe"
out_pdcsta="${cicero_dir}/${group}.pdc.sta"
out_peak2gene="${cicero_dir}/${group}.pdc.peak2gene"
out_gene2peak="${cicero_dir}/${group}.pdc.gene2peak"
out_pdcdist="${cicero_dir}/${group}.pdc.dist"
...
join -1 2 -2 1 <(sort -k2,2 ${fitConn_alignv1_file}) <(sort -k1,1 ${peakannot_file}) -t$'\t' \
    ...
    | sed -e "1i peak1\tcre1\tclass1\tgene1\tpeak2\tcre2\tclass2\tgene2\tcoaccess\tpval\tfdr" > ${out_anno}
...
awk 'BEGIN{FS=OFS="\t"}($3!=$7)' ${out_sel} \
    | awk 'BEGIN{FS=OFS="\t"}(NR>1){if($3=="proximal"){print $0}else{print $5,$6,$7,$8,$1,$2,$3,$4,$9,$10,$11}}' > ${out_pdc}

echo "align v1 pdc format"
awk -v group=${group} 'BEGIN{FS=OFS="\t"} {print $1,$2,$3,$5,$6,$7,$9,$10,$11,group,$4}' ${out_pdc} \
    | sed -e "1i peak1\tcre1\tanno1\tpeak2\tcre2\tanno2\tcoaccess\tpval\tfdr\tgroup\tgene" > ${out_alignv1_pdc}
